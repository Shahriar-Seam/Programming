...
struct proc {
    ...
    proc(int p, int at, int bt, int pr) : p(string("p") + to_string(p)), at(at), bt(bt), pr(pr) {}

    proc(string p, int at, int bt, int pr) : p(p), at(at), bt(bt), pr(pr) {}
...
int32_t main()
{
    ...
    for (i = 0; i < n; i++) {
        cout << q[i].p << " " << q[i].at << " " << q[i].bt << " " << q[i].pr << "\n";
    }
    ...
    for (i = 0, c_t = 0, c_p = {"NA", -1, -1, -1}; i <= total_time; i++) {
        for (auto &it : q) {
            if (it.at == i) {
                s.insert(it);
            }
        ...
        if ((!s.empty() && c_p.pr > s.begin()->pr) || (c_p.bt <= 0)) {
            if (c_p.bt > 0) {
                s.insert(c_p);
            }

            if (c_p.p != "NA") {
                p_ord.push_back(p_time(c_p.p, c_t));
            }
