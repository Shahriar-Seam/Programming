...
bool circle_intersect(pt a, pt b, double r1, double r2, pair <pt, pt> *out)
{
    if (a == b) {
        assert(r1 != r2);
    ...
    double d2 = sq(vec), sum = r1 + r2, diff = r1 - r2;
    double p = (d2 + r1 * r1 - r2 * r2) / (d2 * 2), h2 = r1 * r1 - p * p * d2;

    if (sum * sum < d2 || diff * diff > d2) {
        return false;
    }

    pt mid = a + vec * p, per = perp(vec) * sqrt(fmax(0, h2) / d2);
...
bool possible(int n, vector <pt> &v, double m, pt p)
{
    int i, j;
    pair <pt, pt> out;
    ...
    for (i = 0; i < n; i++) {
        for (j = i + 1; j < n; j++) {
            if (!circle_intersect(v[i], v[j], m, m, &out) || (out.first == out.second)) {
                // ok
            }
            else {
                return false;
