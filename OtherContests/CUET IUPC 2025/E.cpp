...
struct pt {
    ...
    pt (T x = 0, T y = 0, int v = 0) : x(x), y(y), v(v), a(0) {}
...
{
    int n, r, A, i, j;
    int max_sum = 0, min_sum = 0, sum_p = 0, sum_n = 0, sum = 0;
    double theta;

    cin >> n >> r >> A;

    theta = min(2 * PI, A * 2.0 / (r * r));
    ...
    for (auto &it : v) {
        ...
        it.a = atan2(it.y, it.x);

        if (it.a < 0) {
            it.a += 2 * PI;
        }

        max_sum = max(max_sum, it.v);
        min_sum = min(min_sum, it.v);
    }
    ...
    for (i = 0, j = 0; i < 2 * n; ) {
        ...
        while (j < 2 * n && j - i < n && leq(angle(v[i], v[j]), theta)) {
            sum_p += v[j].v;
